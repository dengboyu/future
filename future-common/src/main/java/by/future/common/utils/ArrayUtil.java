...
public class ArrayUtil {
    ...
    /*
     * 从N个数中随机获取length个不重复的数
     *
     * @Author：by@Deng
     * @Date：2018/4/12 15:18
     */
    public static Set<Integer> getDiffNumRandom(int seed,int length){

        Random rand = new Random();
        Set<Integer> numSets = new HashSet<>();

        do{
            numSets.add(rand.nextInt(seed));
        }while(numSets.size()<length);
